...
@Component({
  ...
  templateUrl: './sub-category-new.component.html',
  styleUrls: ['./sub-category-new.component.css']
})
export class SubCategoryNewComponent implements OnInit {
  ...
  constructor(private title: Title, private meta: Meta, private scroller: ViewportScroller, private activatedRoute: ActivatedRoute,
    private sharedService: SharedService, private categoryService: CategoryService, private locationService: LocationService,
    private ls: LocalStoreService, private router: Router,
  ...
  getSubCategiryServiceList(sub_category_id: any) {
    return this.SubCategoryServicesList.filter((x: any) => x.sub_category_id === sub_category_id);
  }
  ...
  SubCategory(p_category_id: any, p_sub_category_id: any) {
    ...
    this.categoryService.SubCategory(model).subscribe((data: any) => {
      ...
      if (p_category_id == 30 || p_category_id == 4 || p_category_id == 46 || p_category_id == 52) {
        this.categoryService.GetCourseModalData(p_category_id).subscribe((x) => {
          let SubCatId = 0;
