...
export class ModifyThisBookingsComponent implements OnInit {
  ...
  if(IsValid==1){
    ...
    this.categoryService.CancelOrder(model).subscribe((data:any)=>{
      ...
      this.sharedService.encryptData(model).subscribe((en:any)=>{
        let sms={
     
          p_msg:"Your appointment with Mirrors beauty Lounge on "+this.selectedSlotDate+" at "+this.selectedTime+" has been cancelled. Thank You!",
  ...
  this.IsLoading=true;
    ...
    this.categoryService.Reschedule(model).subscribe((data:any)=>{
      this.IsLoading=false;
      if(data[0].code=='1'){
        let modelencryptData={
          value: this.orderId
        }
        this.sharedService.encryptData(modelencryptData).subscribe((en:any)=>{
          let sms={
     
            p_msg:"Your appointment with Mirrors beauty Lounge on "+this.selectedSlotDate+" at "+this.selectedTime+" has been rescheduled. Thank You!",
            p_mobile_number:this.ls.getItem('customer')[0]["phone"]
          }
          this.categoryService.SMS(sms).subscribe((x:any)=>{
          this.sharedService.Message(data[0].msg,'success');
